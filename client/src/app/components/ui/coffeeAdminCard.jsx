...
const CoffeeAdminCard = () => {
  ...
  const created = timeCanger(coffeeItem.createdAt);
  const updated = timeCanger(coffeeItem.updatedAt);
  ...
  const handleDelete = async () => {
    for (const key in coffeeItem.images) {
      if (coffeeItem.images[key]) {
        const { message } = await fileService.remove(coffeeItem.images[key]);
        console.log(message);
      }
    }
  ...
  return (
    <div
      className="div m-auto w-75  text-center shadow p-2"
      style={{ width: "350px" }}
    >
      ...
      <div className="mt-4 text-end">
        <Link className="me-4" to={`/adminPanel/coffee/${coffeeItem._id}/edit`}>
